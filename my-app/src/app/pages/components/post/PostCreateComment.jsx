...
import { newCommentIsAdded } from "../../../../features/post/PostCommentsSlice";
import { createNewComment } from "../../../../features/post/PostCommentsApi";
...
export function PostCreateComment({ postCommentRoom }) {
  ...
  useEffect(() => {
    if (postCommentRoom) {
      ...
      console.log("new connection for posts");
    }
    return () => {
  ...
  useEffect(() => {
    ...
    if (postCommentRoom && socket.current) {
      socket.current.on("newCommentAdded", (newComment) => {
        console.log("new comment this ==== ", newComment);
        Dispatch(newCommentIsAdded(newComment));
      });
    }
    return () => {
      socket.current.off("newCommentAdded");
    };
  }, []);
